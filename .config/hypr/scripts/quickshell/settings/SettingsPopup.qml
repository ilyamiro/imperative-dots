...
Item {
    ...
    function scrollHighlightedIntoView() {
        ...
        if (root.currentTab === 0 && generalLoader.item) {
            ...
            let approxY = box * root.s(56) + root.s(80);
    ...
    function saveAppSettings() {
        ...
        let guideVal = root.setOpenGuideAtStartup;
        let patchCmd = guideVal
            ? "sed -i 's|^#*[[:space:]]*exec-once = ~/.config/hypr/scripts/qs_manager.sh toggle guide.*|exec-once = ~/.config/hypr/scripts/qs_manager.sh toggle guide \\&|' ~/.config/hypr/config/autostart.conf"
            : "sed -i 's|^exec-once = ~/.config/hypr/scripts/qs_manager.sh toggle guide.*|# exec-once = ~/.config/hypr/scripts/qs_manager.sh toggle guide \\&|' ~/.config/hypr/config/autostart.conf";
        Quickshell.execDetached(["bash", "-c", patchCmd]);
    ...
    Process {
        ...
        command: ["bash", "-c", "grep -E '^\\s*exec-once\\s*=' ~/.config/hypr/config/autostart.conf 2>/dev/null | sed 's/^\\s*exec-once\\s*=\\s*//'"]
        running: false
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                for (let line of lines) {
                    line = line.trim();
                    if (line.length > 0) {
                        dynamicStartupModel.append({ command: line, isEditing: false });
                    }
                }
                root.saveAllStartup();
            }
    ...
    function saveAllStartup() {
        ...
        let jsonStr = JSON.stringify(startupArray).replace(/'/g, "'\\''");
        let cmd = "mkdir -p ~/.config/hypr/ && [ ! -f ~/.config/hypr/settings.json ] && echo '{}' > ~/.config/hypr/settings.json; " +
                  "jq '.startup = " + jsonStr + "' ~/.config/hypr/settings.json > ~/.config/hypr/settings.json.tmp && " +
                  "mv ~/.config/hypr/settings.json.tmp ~/.config/hypr/settings.json && " +
                  "notify-send 'Quickshell' 'Startup entries saved!'";
        Quickshell.execDetached(["bash", "-c", cmd]);
    }
        ...
        onTriggered: {
            if (targetBox >= 0) {
                ...
                if (targetTab === 0 && generalLoader.item) {
                    ...
                    approxY = targetBox * (root.s(56)) + root.s(80);
            ...
            onStreamFinished: {
                try {
                    if (this.text && this.text.trim().length > 0 && this.text.trim() !== "{}") {
                        ...
                        if (parsed.startup !== undefined && Array.isArray(parsed.startup)) {
                            dynamicStartupModel.clear();
                            let tempStartup = [];
                            for (let s of parsed.startup) {
                                tempStartup.push({ command: s.command || "", isEditing: false });
                            }
                            dynamicStartupModel.append(tempStartup);
                        } else {
                            autostartMigrator.running = true;
                        }
                    } else {
            ...
            ColumnLayout {
                ...
                Item {
                    ...
                    Rectangle {
                        ...
                        property real tabW: (parent.width - root.s(6)) / 4
